...
class LongVideoBenchDataset(Dataset):
    def __init__(self, dataset_path: str, sample_config: dict):
        ...
        if 'filtered' not in sample_config or sample_config['filtered'] != True:
            ...
            json_path = osp.join(dataset_path, "lvb_val_filtered.json")
        
        self.dataset_path = osp.join(dataset_path, "videos")
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if os.path.exists(full_video_path):
            ...
            # folder_path = osp.join(self.dataset_path, video_path)
        ...
        for i in range(len(question_data)):
            ...
            ] + [f"{chr(65 + j)}. {question_data[i]['candidates'][j]}" for j in range(len(question_data[i]['candidates']))])
            question_data[i]["text"] = text

            question_data[i]['answer'] = chr(65 + question_data[i]['correct_choice'])
