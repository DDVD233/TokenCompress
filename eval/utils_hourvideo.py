...
def load_decord(src_path: str, sample_type: str, **kwargs) -> list[Image.Image]:
    """Load video using torchvision, optionally load subtitles
    ...
    if sample_type == 'uniform':
        ...
        frames = selected_frames.permute(0, 2, 3, 1).numpy().astype(np.uint8)
...
def load_folder(src_path: str, sample_type: str, **kwargs) -> list[Image.Image]:
    """Load video frames from folder, optionally load subtitles
    ...
    def __init__(self, dataset_path: str, json_path: str, sample_config: dict):
        ...
        with open(json_path, "r") as f:
            ...
            for video_id, content in self.full_data.items():
                video_info = {
                    "video_uid": video_id,
                    # "metadata": content.get("video_metadata", {}),
                    "benchmark_dataset": content.get("benchmark_dataset", [])
                }
                self.full_data_list.append(video_info)

    def __len__(self):
        return len(self.full_data_list)

    def __getitem__(self, idx) -> dict:
        if os.path.exists(osp.join(self.dataset_path, self.full_data_list[idx]['video_uid']+'.mp4')):
            frames, durations = load_decord(
                src_path=osp.join(self.dataset_path, self.full_data_list[idx]['video_uid']+'.mp4'),
                **self.sample_config
            )
        else:
            pass
        questions = self.full_data_list[idx]["benchmark_dataset"]
        

        return dict(
            uid=self.full_data_list[idx]['video_uid'],
            video=frames,
            questions=questions,
        )